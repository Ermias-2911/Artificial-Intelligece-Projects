...
def enhancedFeatureExtractorDigit(datum):
    ...
    while i<len(pix):
        ...
        while j<len(pix[i]):
            if pix[i][j]==0:
                ...
                no_of_nonzero_pixels+=1
                if j<centerpixels:
                    centerpixels=j
                if not centerpixels:
                    centerpixels=j
                length_of_pix=(len(pix)+1)    
                if (length_of_pix/2)>=j:
                    counter_cen+=1
            if pix[i][j-1]==pix[i][j]:
                pass
            else:    
                no_of_breaks+=1
            j+=1
    ...
    while j<len(pix[0]):
        ...
        while i<len(array):
            if array[j]==0:
                ...
                if not random_pixel:
                    random_pixel=i
                length_of_pix=(len(pix[0])+1) 
                if (length_of_pix+1)/2>=i:
                    counter+=1
            ...
            i+=1
        j+=1     
    hei=len(pix)-(random_pixel*2)
    Asp_ratio=float(wid)/hei
    i=0
    while i<5:
        if no_of_breaks>200:
            features[i]=1.0
    ...
    i=0
    while i<5:
        if no_of_nonzero_pixels>300:
            features[-i]=1.0
        else:
            features[-i]=0.0
        i+=1    
    active_pixels_above=float(counter_cen)/no_of_nonzero_pixels
    i=0
    while i<5:
        if active_pixels_above>0.4:
            features[-(i+1)*10]=1.0
        else:
            features[-(i+1)*10]=0.0
        i+=1
    active_pixels_right=float(counter)/no_of_nonzero_pixels
    i=1000
    while i<1005:
        if active_pixels_right<0.3:
            features[i]=1.0
        else:
            features[i]=0.0
        i+=1
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    gen_successor=state.generateSuccessor(0,action)
    get_pac_position=gen_successor.getPacmanPosition()
    get_ghost_position=gen_successor.getGhostPositions()
    get_capsule=gen_successor.getCapsules()
    food=[]
    Food_st=state.getFood()
    i=0
    for x in Food_st:
        for y,food1 in enumerate(x):
            if(food1):
                food.append((i,y))
        i+=1
    list1=[]
    for i in get_ghost_position:
        list1.append(util.manhattanDistance(get_pac_position, i))
    nearest_ghosts=sorted(list1)
    features["nearest_ghost"]=nearest_ghosts[0]*1.0
    jj=0
    tys=min(len(nearest_ghosts),1)
    while jj<tys:
        features[("ghost", jj)]=5/(0.1+nearest_ghosts[jj])
        jj+=1
    list2=[]
    for i in get_capsule:
        list2.append(util.manhattanDistance(get_pac_position,i))
    nearest_caps=sorted(list2)
    jj=0
    tys=min(len(nearest_caps), 1)
    while jj<tys:
        features[("capsule",jj)]=15/(1+nearest_caps[jj])
        jj+=1
    list3= []
    for i in food:
        list3.append(util.manhattanDistance(get_pac_position,i))
    nearest_food=sorted(list3)
    ...
    features["capsule count"]=len(get_capsule)*10
    #util.raiseNotDefined()
    return features
