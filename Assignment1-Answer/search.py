...
def depthFirstSearch(problem):
    ...
    while not state_stack.isEmpty():
        ...
        popped_state, popped_moves = state_stack.pop()
        ...
        for state, direction, cost in problem.getSuccessors(popped_state):
            if (state in set_states):
                continue
            state_stack.push((state, popped_moves + [direction]))
...
def uniformCostSearch(problem):
    ...
    set_states = set()
    state_pq.push((problem.getStartState(), [], 0), 0)

    while not state_pq.isEmpty():

        popped_state, popped_moves, popped_cost = state_pq.pop()
        ...
        for state, direction, cost in problem.getSuccessors(popped_state):
            if (state in set_states):
                continue
            state_pq.push((state, popped_moves + [direction], popped_cost + cost), popped_cost + cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not state_pq.isEmpty():
        ...
        for state, direction, cost in problem.getSuccessors(popped_state):
            ...
            state_pq.push((state, popped_moves + [direction], popped_cost + cost), popped_cost + cost + hvalue)
    return []
