...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        for crn in self.corners:
            if self.startingPosition == crn:
                self.cornersVisited.append((crn, True))
            else:
                self.cornersVisited.append((crn, False))
...
def cornersHeuristic(state, problem):
    ...
    for c in corner_state:
        pos = state[0]
        cor_position = c[0]
        if c[1] == False:
            dist = max(dist, (abs(cor_position[0] - pos[0]) + abs(cor_position[1] - pos[1])))
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    pmc_pos, food_grd = state
    max_dist = 0
    for food_pos in food_grd.asList():
        xy1 = food_pos
        xy2 = pmc_pos
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        while not state_queue.isEmpty():
            ...
            popped_state, popped_moves, popped_cost = state_queue.pop()

            if (popped_state in expanded):
                continue

            if problem.isGoalState(popped_state):
                return popped_moves

            expanded.add(popped_state)

            for state, direction, cost in problem.getSuccessors(popped_state):
                if (state in expanded):
                    continue
                state_queue.push((state, popped_moves + [direction], popped_cost))
