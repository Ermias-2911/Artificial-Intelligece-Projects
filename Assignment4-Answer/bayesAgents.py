...
def fillYCPT(bayesNet, gameState):
    ...
    temp = bn.Factor([Y_POS_VAR], [], bayesNet.variableDomainsDict())
    "*** YOUR CODE HERE ***"
    temp.setProbability({Y_POS_VAR: BOTH_TOP_VAL}, PROB_BOTH_TOP)
    temp.setProbability({Y_POS_VAR: BOTH_BOTTOM_VAL}, PROB_BOTH_BOTTOM)
    temp.setProbability({Y_POS_VAR: LEFT_TOP_VAL}, PROB_ONLY_LEFT_TOP)
    temp.setProbability({Y_POS_VAR: LEFT_BOTTOM_VAL}, PROB_ONLY_LEFT_BOTTOM)
    bayesNet.setCPT(Y_POS_VAR, temp)
...
class VPIAgent(BayesAgent):
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for x, y in self.getExplorationProbsAndOutcomes(evidence):
            tempVar = evidence.copy()
            tempVar.update(y)
            y, z = self.computeEnterValues(tempVar, enterEliminationOrder)
            c = max(y, z)
            val += c * x
        return val
