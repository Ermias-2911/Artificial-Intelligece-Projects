...
def joinFactors(factors):
    ...
    if factors and len(factors) > 0:
        val = factors[0].variableDomainsDict()

    for i in factors:
        t = i.unconditionedVariables()
        r = i.conditionedVariables()
        arr.extend(t)
        for j in r:
            if j not in arr1:
                arr1.append(j)
    arr1 = [var for var in arr1 if var not in arr]

    x = Factor(arr, arr1, val)
    z = x.getAllPossibleAssignmentDicts()
    for i in z:
        prob = 1
        for factor in factors:
            prob *= factor.getProbability(i)
        x.setProbability(i, prob)
...
def normalize(factor):
    ...
    # you should return None.
    variableDomainsDict = factor.variableDomainsDict()
    val = factor.unconditionedVariables()
    temp = factor.conditionedVariables()

    t = 0
    z = factor.getAllPossibleAssignmentDicts()
    for i in z:
        t += factor.getProbability(i)
    if (t == 0):
        return None

    for j in val:
        if len(variableDomainsDict[j]) == 1:
            temp.add(j)
    val = [j for j in val if j not in temp]
    result = Factor(val, temp, variableDomainsDict)

    for i in result.getAllPossibleAssignmentDicts():
        r = factor.getProbability(i)
        result.setProbability(i, r / t)
    return result
